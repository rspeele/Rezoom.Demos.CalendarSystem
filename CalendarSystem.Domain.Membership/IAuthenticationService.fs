...
type IAuthenticationService =
    ...
    // you could change the system to use Guids or something for session IDs. Sessions *do* need
    // an ID separate from the token though, because we want to expose that ID as part of `Occurence` without
    // leaking session tokens to anybody.
